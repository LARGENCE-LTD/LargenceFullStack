...
const useToolbarVisibility = (
  ...
  const checkVisibility = React.useCallback(() => {
    ...
    // Check if toolbar has any groups at all
    const hasGroups = Array.from(toolbar.children).some((child) => {
      if (!(child instanceof HTMLElement)) return false;
      return child.getAttribute("role") === "group";
    });

    // Only hide if there are no groups at all, not if groups are empty
    setIsVisible(hasGroups);
  }, [ref]);
...
const useGroupVisibility = (
  ...
  const checkVisibility = React.useCallback(() => {
    ...
    // Always show groups that exist, don't hide based on children
    setIsVisible(true);
  }, [ref]);
