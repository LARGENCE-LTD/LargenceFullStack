...
export function useUserActions(state: UserState, dispatch: Dispatch<UserAction>) {
  ...
  const loadUserData = async (token: string) => {
    try {
      ...
      const [profile, subscription, usage, preferences, activities] = await Promise.all([
      ...
      dispatch({ type: USER_ACTION_TYPES.SET_SUBSCRIPTION, payload: subscription });
      dispatch({ type: USER_ACTION_TYPES.SET_USAGE, payload: usage });
      dispatch({ type: USER_ACTION_TYPES.SET_PREFERENCES, payload: preferences });
      dispatch({ type: USER_ACTION_TYPES.SET_ACTIVITIES, payload: activities });

      // Update last activity
      dispatch({ type: USER_ACTION_TYPES.UPDATE_LAST_ACTIVITY, payload: new Date().toISOString() });
  ...
  // Profile management
  const updateProfile = async (profileData: Partial<{ firstName: string; lastName: string; email: string }>) => {
    ...
    try {
      const updatedProfile = await UserAPI.updateProfile(state.token, profileData);
      dispatch({ type: USER_ACTION_TYPES.SET_PROFILE, payload: updatedProfile });
  ...
  const updateSubscription = async (planId: string) => {
    ...
    try {
      const updatedSubscription = await UserAPI.updateSubscription(state.token, planId);
      dispatch({ type: USER_ACTION_TYPES.SET_SUBSCRIPTION, payload: updatedSubscription });
      
      addActivity('subscription_changed', `Subscription updated to ${updatedSubscription.plan.name}`);
  ...
  const updatePreferences = async (preferences: Partial<{
    ...
    try {
      const updatedPreferences = await UserAPI.updatePreferences(state.token, preferences);
      dispatch({ type: USER_ACTION_TYPES.SET_PREFERENCES, payload: updatedPreferences });
  ...
  // Activity management
  const addActivity = (type: 'login' | 'logout' | 'document_created' | 'document_exported' | 'profile_updated' | 'subscription_changed', description: string, metadata?: Record<string, any>) => {
    const activity = {
  ...
  const initializeFromStorage = async () => {
    ...
    if (savedPreferences) {
      dispatch({ type: USER_ACTION_TYPES.SET_PREFERENCES, payload: savedPreferences as UserPreferences });
    }
  };
